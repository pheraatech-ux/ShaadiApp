...
export function StickyNoteCard({
  ...
  function onDragPointerDown(event: React.PointerEvent<HTMLDivElement>) {
    if (!note.isCurrentUser || editing) return;
    if ((event.target as HTMLElement).closest("textarea, button, [data-resize-handle]")) return;
    event.stopPropagation();
  ...
  return (
    ...
    >
        ...
        <>
          ...
          >
            ...
            <div className="relative">
              ...
              {showPalette && (
                <div className="absolute right-0 top-8 flex gap-1.5 rounded-xl border border-border/60 bg-popover p-2 shadow-lg">
      ...
      <div
        className={cn(
          "flex h-full cursor-default flex-col rounded-2xl border p-4 shadow-md transition-shadow group-hover:shadow-lg",
          colors.card,
          colors.border,
          note.isCurrentUser && !editing && "cursor-grab active:cursor-grabbing",
        )}
      ...
      >
        {editing ? (
          ...
          <p
            className={cn(
              "flex-1 overflow-hidden whitespace-pre-wrap text-sm leading-relaxed text-foreground/80",
        ...
        <div className="mt-auto flex items-end justify-between gap-2 border-t border-current/10 pt-2">
        ...
        {note.isCurrentUser && (
          <div
            ...
            className="absolute bottom-0 right-0 cursor-se-resize p-2 opacity-0 transition-opacity group-hover:opacity-100"
